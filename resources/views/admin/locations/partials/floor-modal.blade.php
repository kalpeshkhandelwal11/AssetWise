<div x-data="{
     ...
     @open-floor-modal.window="open($event.detail)"
     x-show="showModal"
    ...
    <div @click.outside="showModal = false"
        ...
        <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-1" x-text="editId ? 'Edit Floor' : 'New Floor'"></h2>
        <p class="text-sm text-gray-400 mb-4">Building: <span class="font-medium" x-text="buildingName"></span></p>

        <template x-if="!editId">
            <form method="POST" :action="`{{ url('admin/buildings') }}/` + buildingId + '/floors'" class="space-y-4">
                @csrf
                <div>
                    <x-input-label value="Floor Name *" />
        ...
        <template x-if="editId">
            <form method="POST" :action="`{{ url('admin/floors') }}/` + editId" class="space-y-4">
                @csrf @method('PUT')
                <div>
                    <x-input-label value="Floor Name *" />
                    <x-text-input name="name" class="mt-1 block w-full" x-model="form.name" required autofocus />
