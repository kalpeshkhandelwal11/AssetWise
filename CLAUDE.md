...
| M07 Shared UI & Services | ✅ done | Reusable Blade components (`x-data-table`, `x-filter-bar`, `x-status-badge`, `x-breadcrumb`, `x-confirm-modal`) retrofitted across all M01–M06 list/filter screens, eliminating duplicated table/filter markup; service interfaces (`WorkflowService`, `NotificationService`) already documented via M08. Sidebar nav (`layouts/sidebar-nav.blade.php`) was likewise extracted into `x-sidebar-link`/`x-sidebar-group`/`x-sidebar-section` — labeled sections, `x-collapse` expand/collapse, `aria-expanded`/`aria-controls`/`aria-current` — replacing ~15 copies of the same hover/active class string. The desktop collapse button in `layouts/app.blade.php` uses a single panel/divider icon (`aria-controls="app-sidebar"`) whose Tailwind text-shade reacts to `sidebarCollapsed` via Alpine `:class` |
...
| M14 Reports & Dashboard | ✅ done | Company-scoped dashboard (`DashboardService`) plus 11 exportable reports (Asset Register, Movement, Inter-Company Transfer, Disposal, Maintenance, AMC & Warranty, Asset Aging, Utilization, Audit/Compliance, Audit Campaign, Depreciation Schedule) via `ReportService` + `ReportController` (`Reports/` namespace) + `ReportRegistry`. Excel via Maatwebsite Excel, PDF via a shared `ReportPdfExporter`/generic table view, large exports queued through `GenerateReportExport` (mirrors M06's `GenerateAssetExport`) with `NotificationService` "export ready" notify. Maintenance Report (M11 service history) and AMC & Warranty (a UNION of `amc_contracts`/`warranty_records` via the read-only `App\Models\Reports\CoverageContract`, one row per contract) both shipped in an M14 follow-up once M11 landed — **every filter on the AMC & Warranty union must be applied to both legs before `->union()`, never once on the combined builder, or company-scoped rows from the second leg leak unfiltered** (see the gotcha below). Asset Register also gained `is_eol`/`eol_projected_date` columns, which — since `AssetExport` is shared — changed M06's asset export too. Kit Assignment History remains registered in `ReportRegistry` as a disabled "coming soon" entry pending M17 (checklist to enable it lives in `M17-asset-kits.md`) |
...
| M12 Notifications | ✅ done | Central type catalog (`config/notifications.php` + `App\Support\NotificationCatalog`) drives label/title/body/link/channels for all 11 live type strings — `NotificationService::send/sendMany($user, $type, $data)` stays the unchanged cross-module contract, now dispatching two notification classes: `GenericNotification` (database channel, inline, **not** `ShouldQueue`) and `GenericMailNotification` (mail channel, **`ShouldQueue`**). Per-user `notification_preferences` (sparse — a row only when a user's choice differs from the catalog default) gate the mail channel via `User::wantsEmailFor()`; a Notifications tab on `/profile` edits them. Bell dropdown and `/notifications` index render real rows and mark-as-read now deep-links via each type's `link` resolver (a payload key, or a closure for `export_ready`/`import_completed`, which don't carry a `url` key). No new RBAC permission — scoped by `auth()->user()->notifications()`, not `{module}.{action}` |
| M17 Asset Kits & Bundles | ✅ done | Kit templates (`kits`/`kit_items`/`kit_assets`) + ad-hoc bundles, assigned in one action via **any non-return M09 movement type** (assign-to-custodian, relocate, inter-company). `KitService` = plain `kits.manage` template CRUD with category-match slot validation + readiness. `KitAssignmentService` drives M09: the global `kit_assignment_approval_mode` setting picks **`single`** (one `kit_assignments` header is the approvable on a new `kit_assignment` workflow → `ApplyKitAssignment` runs `MovementService::applyBulk()`; `MarkKitAssignmentRejected` frees the assets) or **`per_asset`** (one `MovementService::submit()` → `transfer` approval each). **Reuses M09's `AssetMovementBatch`** (`kit_assignment_id`) rather than a `kit_assignment_items` table — `MovementService::submitBatch()` was refactored to extract a workflow-free `buildBatch()`. Kit **return** mirrors assign with RETURN movements. Inter-company kit transfer inherits M16's per-asset `resetForTransfer()` through `applyBulk()` for free. `kit_assignment` has **no seeded workflow** (deliberate — admin configures at `/admin/workflows`). Permissions `kits.manage`/`kits.assign`/`kits.view` |

Test suite: **591 passing**. For the full developer setup guide see `docs/developer-setup.md`; for the end-to-end browser integration pass (and the six defects it caught that the suite could not) see `docs/integration-testing.md`.
...
- **Never write JS `//` comments containing a double quote inside an `x-data="..."` attribute.** The attribute is delimited by double quotes, so a literal `"` in a comment (e.g. `// "Refresh" affordance…`) closes the attribute early, truncating the whole object literal. Alpine then fails with `Alpine Expression Error: Unexpected token` and the **entire component silently dies** — every `@click`, `x-model`, `:href`, and auto-fill on that form stops working at once (this took down the asset-create form's EOL calc, custodian auto-fill, Add-media, Refresh, and Add-Attribute link together). Use Blade `{{-- --}}` comments (stripped at compile, never reach the attribute) for anything inside an `x-data`, and keep literal `"` out of the expression.
...
- **`ShouldQueue` on a multi-channel `Notification` queues *every* channel, including `database`** (M12). Putting it on `GenericNotification` would mean the bell doesn't update until a queue worker runs — never, in a dev environment with none running. That's why the class is split: `GenericNotification` (database, inline, no `ShouldQueue`) and `GenericMailNotification` (mail, `ShouldQueue`). Mail requires `php artisan queue:work` to actually go out — with `QUEUE_CONNECTION=database` and no worker running, the in-app notification still appears but the mail job sits in `jobs` forever, silently.
- **A filter applied before `->union()` constrains only that first leg** (M14, `ReportService::buildAmcWarrantyQuery()`). `$query->whereHas(...)->union($other)` leaves `$other`'s rows completely unfiltered — on a company-scoped report that is a cross-company data leak, not just wrong row counts. Verified while building the AMC & Warranty report: a naive single-filter version let another company's warranty row through. Every filter must be applied to each leg independently (`applyCoverageFilters()`, called once per leg) before the `->union()` call, never once on the combined builder. `AmcWarrantyCompanyScopeTest` fails against the naive version and is the regression guard.
...
### Asset Create Form (status vocabulary, naming series, creation approval)

- **Statuses**: `Inter-Company Transfer` (code `ICT`) displays as **"Sold"** and `Disposed` (code `DISPOSED`) as **"Scrapped"** — display-name renames only; the `code`s are unchanged so `Asset::isDisposed()` and M09/M13 logic hold. A new **`DRAFT`** status backs the creation-approval holding state.
- **Naming series**: `AssetNamingService` auto-generates `assets.asset_tag` as `PREFIX-000N` (default `AST-0001`) on create unless a tag is supplied. Counter is a row-locked `Setting` (`asset_naming_*`), admin-edited at `/admin/settings/asset-naming`. Independent of the M05 physical tag pool (the create form's Barcode section still assigns a pool `Tag` via `TagService::assignToAsset`).
- **Creation approval** (`asset_creation` workflow module): `AssetController::store()` wraps create + submit in a transaction; when an **active** `asset_creation` workflow exists it parks the asset in `DRAFT` and calls `WorkflowService::submit($asset, 'asset_creation', $actor)` (the `Asset` is the approvable), else it saves live. `ApplyAssetCreation` (on `ApprovalRequestApproved`) promotes DRAFT→`AVAILABLE`; `MarkAssetCreationRejected` leaves it DRAFT (resubmit via `assets.submit-approval`). Draft assets are blocked from movement/disposal (`isDraft()` guards in `MovementService::assertMovable` / `DisposalService::assertDisposable`). Media at creation is stored as labelled `AssetAttachment` rows (`type` enum gained `photo`); custodian selection auto-fills Department & Branch from the `Employee`.
